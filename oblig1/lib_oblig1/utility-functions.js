...
/**
 * @param {{ clockRotationHour: number, clockRotationMinute: number }} animations 
 */
export function handleTime(animations) {
  const date = new Date();

  const hours = date.getHours() % 12;
  const minutes = date.getMinutes();

  animations.clockRotationHour = hours * 30 + minutes / 60 * 30;

  animations.clockRotationMinute = minutes * 6;
}
